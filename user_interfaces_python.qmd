...
subtitle: "CLI, TUI, and Web Apps: Click · Typer · Fire · docopt · Rich · tqdm · Flask · Streamlit"
author: "Juan F. Imbet"
institute: "Master 2 (203) Financial Markets — Paris Dauphine · PSL"
date: "2025-10-23"
format:
  revealjs:
    slide-number: true
...
- Understand the Python UI landscape: CLI, TUI, and Web UIs
- Learn modern CLI tooling: Click, Typer, Fire, docopt
- Master terminal UX: colors, progress, animations with Rich and tqdm
- Build simple web UIs with Flask and Streamlit
- Practice professional app structure, packaging, and testing
...
- VS Code/Codespaces setup for UI development
- Best practices, testing, and deployment pointers
...
- CLI: repeatable tasks, scripting, integration, headless servers
- GUI/Web: interactive exploration, broader audience, onboarding
- Cost: CLIs are fast to build; Web adds hosting and state
- Observability: logs vs browser dev tools vs server logs
- Distribution: pip/pipx vs container/URL; security and updates
...
- Prefer explicit options (e.g., --input, --verbose) over positionals
...
parser = argparse.ArgumentParser(prog="demo", description="Demo CLI")
parser.add_argument("path", help="Input file path")
parser.add_argument("--verbose", "-v", action="count", default=0)
args = parser.parse_args()

print(f"argv: {sys.argv}")
print(f"path={args.path}, verbose={args.verbose}")
```

---

## argparse essentials (stdlib)
- Batteries-included, no extra dependency
- Generates --help and usage automatically
- Supports subcommands via subparsers
- Type conversion and validation (choices, types)
- Good baseline; consider Click/Typer for larger apps
...
## Professional CLI UX: essentials
- Consistent command shape: cmd subcmd [OPTIONS] [ARGS]
- Clear help: examples, defaults, environment variables
- Friendly errors: actionable messages, exit codes, --verbose
- Config layering: defaults → env → CLI → config file
- Telemetry and update checks: opt-in, transparent, documented

---

## Auto-update and versioning patterns
- Expose --version and embed git/PEP 440 version in code
- Suggest upgrade path (pipx upgrade, brew upgrade, self-update)
- Optional update check with cache (daily) and timeout guards
- Respect proxies/offline; never block main execution
- Log to stderr; fail-soft on network errors

---

## Colors, styles, and layout in terminal
- Use Rich for colors, markup, tables, panels, trees
- Prefer semantic styles over hard-coded ANSI
...
## Animations and spinners (polish)
...
- Consider CI detection to disable animations
...
```python
from tqdm import tqdm
with tqdm(total=100, desc="Step") as bar:
    for _ in range(20):
        bar.update(5)
```
...
## Click: overview
- Mature, decorator-based CLI framework
- Commands, groups, options, prompts, validation
- Auto help, colors, environment variable support
- Context object for shared state and settings
- Great for medium–large applications
...
def run(limit):
    click.echo(f"Running with limit={limit}")
...
## Click: options, prompts, and types
...
## Typer: overview
- Built on Click; uses type hints for parsing
- Auto help, completion, and rich error messages
- Great DX: minimal boilerplate and IDE hints
- Scales from scripts to multi-command apps
- My default for new CLI projects
...
class Tools:
    ...
    def greet(self, name="World"): return f"Hi {name}!"
if __name__ == "__main__":
    fire.Fire(Tools)
```
...
- pipx runs Python CLIs in isolated virtualenvs and exposes them on your PATH (great for end users)
- Provide Homebrew or standalone binaries if you target macOS or no-Python hosts
- Version with SemVer (PEP 440), changelog, and minimal pinning for stability
- Document install, upgrade, and uninstall workflows clearly
...
[project.scripts]
mytool = "pkg.module:app"
```

---

## What is pyproject.toml?
- A standardized config file (PEP 518/517/621) for Python projects
...
## What is pipx?
- A tool to install and run Python applications in isolated environments
- Installs each app into its own virtualenv and puts shims on your PATH
- Ideal for end-user CLIs (avoids dependency conflicts with system/site)
- Usage: pipx install mytool · pipx upgrade mytool · pipx uninstall mytool
- Complements, not replaces, pip/conda for libraries used inside projects
...
def summarize(path: str, delim: str = ","):
    try:
        rows = list(track(csv.DictReader(open(path), delimiter=delim),
                          description="Reading"))
    ...
    if rows: table.add_row(*[str(len(rows)) for _ in cols])
    console.print(table)
```
...
- Templates in templates/ directory by default
- Use layout.html and blocks to DRY markup
- WTForms or simple form parsing via request.form
- Static assets via static/ path prefix
- Use app.config for secrets and runtime settings
...
if st.button("Greet"): st.success(f"Hello {name}")
```

---

## Streamlit: layout, state, caching
...
- Use experimental_rerun for flow control when needed
...
- Keep setup simple: pyproject + pipx
